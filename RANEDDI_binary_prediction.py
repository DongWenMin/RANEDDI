...
class RANEDDI(object):
    ...
    def _build_weights(self):
        ...
        all_weights['relation_matrix'] = tf.Variable(initializer([30,2*self.kge_dim,2*self.kge_dim]),
    ...
    def _build_loss_phase_I(self):
        ...
        #该部分损失进行修改
        margin = 1
        maxi = tf.log(tf.clip_by_value(tf.nn.sigmoid(margin-neg_scores),1e-8,1.0)) + tf.log(tf.clip_by_value(tf.nn.sigmoid(pos_scores-margin),1e-8,1.0))
    ...
    def _create_bi_interaction_embed(self):
        ...
        for i in range(self.n_relations):
          ...
          relation_u_neigh = tf.sparse_tensor_dense_matmul(self.sparse_adj_list[i],weight_entity_embedding)
          ...
          relation_e_neigh = tf.sparse_tensor_dense_matmul(tf.sparse_transpose(self.sparse_adj_list[i]),weight_drug_embedding)
